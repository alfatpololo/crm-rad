...
import { deleteCategory } from '@/actions/masterData';
    ...
    const handleAction = async (action, rowData) => {
        if (action === 'Delete') {
            const result = await Swal.fire({
                ...
                text: `Anda akan menghapus kategori "${rowData.name}". Tindakan ini tidak dapat dibatalkan!`,
                icon: 'warning',
            ...
            if (result.isConfirmed) {
                setLoading(true);
                try {
                    const deleteResult = await deleteCategory(rowData.id);
            ...
            router.push(`/master-data/categories/edit/${rowData.id}`);
        }
    };

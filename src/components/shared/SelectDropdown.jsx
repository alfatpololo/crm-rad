...
const SelectDropdown = ({ options, selectedOption, onSelectOption, className, defaultSelect, name }) => {
    ...
    return (
        <div className={`select-dropdown ${className ? className : ""} ${openUpwards ? 'open-upwards' : ''}`} ref={dropdownRef}>
            {name && <input type="hidden" name={name} value={localSelectedOption?.value || ''} />}
